...
//     const targetDistance = Math.ceil(distance - local.nearbyThreshold);
//     const ratio = targetDistance / distance;
//     let targetX = ratio * dx;
//     let targetY = ratio * dy;
//     let targetTries = 0;
...
//       targetTries += 1;
//       const target = randomPointInCircle(item.x, item.y, local.nearbyThreshold);
//       targetX = target[0];
//       targetY = target[1];
//     }
...
//     // if (local.isNear(targetX, targetY)) {
//     if ((local.x === targetX && local.y === targetY) || local.isNear(item.x, item.y)) {
...
//     // if (destination && distanceBetween(destination.x, destination.y, targetX, targetY) <= local.nearbyThreshold) {
//     if (destination && distanceBetween(destination.x, destination.y, item.x, item.y) <= local.nearbyThreshold) {
//       this.succeed();
//       return;
//     }
...
//       this.succeed();
//     } else {
//       // console.log("unreachable; pushing", targetX, targetY);
